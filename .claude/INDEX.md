...
### 1. **Context Reading Order**
```
0. URGENT_READ_FIRST.md - Check this FIRST (if exists)
1. INDEX.md (this file) ← YOU ARE HERE
2. STANDARDS.md - Development rules & standards
3. INITIAL.md - Project specifications & protocols
4. ../PLANNING.md - Architecture & workflow
5. ../TASK.md - Current sprint work
```

**Note on Automated Context:** Some AI assistants, like Claude, may offer slash commands (e.g., `/context-init`) for automated context loading. If available, you can use them. Otherwise, please follow the manual reading order above.
...
#### 🐛 **Bug Fixes**
```
STANDARDS.md → ../TASK.md → test files → source code
```
...
#### 🧪 **Testing & Quality**
```
STANDARDS.md → VALIDATION.md → test files
```
...
- `.claude/STANDARDS.md` - Development standards, testing requirements, Git workflow
...
- `agents/DATABRICKS_GENIE_AGENT.md` - Databricks Genie API expert (1000+ lines)
- `agents/MCP_ARCHITECT_AGENT.md` - MCP server architecture expert (1000+ lines)
- `agents/SQL_SECURITY_AGENT.md` - SQL security & governance expert (1000+ lines)
...
1. `STANDARDS.md` - Component standards and naming conventions
2. `PLANNING.md` - Frontend architecture patterns
3. Project source files
4. `../TASK.md` - Frontend-related current tasks

### **Backend Development**
1. `STANDARDS.md` - Backend standards and testing requirements
...
Activate specialized AI agents for focused technical assistance. If your AI assistant supports slash commands, you can use the following:
...
**Data Platform & MCP Agents:**
```bash
/agent-genie          # Databricks Genie API & NL-to-SQL expert
/agent-mcp            # MCP server architecture expert
/agent-sql-security   # SQL security & data governance expert
```
...
```
If slash commands are not available, you can manually load the agent's context by reading the corresponding file from the `.claude/agents/` directory.
...
3. **Activate**: `/agent-[name]` (if available) or read `agents/[NAME]_AGENT.md`
...
```
/agent-genie → /agent-mcp → /agent-sql-security
```
Build secure MCP servers that connect to Databricks Genie for NL-to-SQL queries
...
- Databricks Genie API: `/agent-genie`
- MCP server architecture: `/agent-mcp`
- SQL injection & data governance: `/agent-sql-security`
...
### **File Authority**
- The `.claude/` directory contains the authoritative context files.
...
4. **Review `STANDARDS.md` for development standards**
...
**Context Version:** 3.1
**Features:** Core context + RAG system + 16 specialized agents (5 technology + 3 data platform/MCP + 8 refactoring)