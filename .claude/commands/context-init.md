...
startup_script = Path(project_root) / ".claude" / "rag" / "startup_display.py"
...
project_root = os.getcwd()
marker_file = Path(project_root) / ".claude" / ".initialized"
bootstrap_script = Path(project_root) / ".claude" / "rag" / "bootstrap.py"
...
status_file = Path(".claude/BOOTSTRAP_STATUS.json")
...
- [ ] Bootstrap completed (check .claude/.initialized)
...
### "Bootstrap failed"
Check `.claude/BOOTSTRAP_STATUS.json` for error details.
