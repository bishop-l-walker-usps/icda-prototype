...
def get_rag_stats(project_root: str) -> Dict[str, Any]:
    ...
    status_file = root / ".claude" / "BOOTSTRAP_STATUS.json"
    ...
    chroma_dir = root / ".claude" / "rag" / "chroma_db"
...
def get_context7_stats(project_root: str) -> Dict[str, Any]:
    ...
    # Check cache
    cache_file = root / ".claude" / "rag" / "context7_cache" / "cache.json"
...
def display_startup_banner(project_root: str, use_colors: bool = True):
    ...
    if not rag_stats["initialized"]:
        ...
        print(f"   > python .claude/rag/bootstrap.py --force - Re-index codebase")
        print(f"   > python .claude/rag/adaptive_rag.py --conventions - Analyze conventions")
    print()
