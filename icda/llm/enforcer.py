"""
LLM Enforcer - Main Integration Class.

Coordinates all three enforcement levels using any LLM provider.
Provider-agnostic replacement for GeminiEnforcer.
...
class LLMEnforcer:
    """
    Main LLM Enforcer - coordinates all 3 levels of enforcement.

    Works with any LLM provider (Gemini, OpenAI, Claude, etc.)
    ...
    Usage:
        ...
        # Explicit provider
        enforcer = LLMEnforcer(provider="openai")
    ...
    def __init__(
        ...
        """
        Initialize the LLM Enforcer.

        Args:
            client: Pre-configured LLM client (optional)
            provider: Provider name if no client given ("auto", "gemini", "openai", "claude")
        ...
        self._metrics = EnforcerMetrics(
            ...
            model=self.client.config.model if self.client and self.client.config else "none",
        )

        if self.available:
            logger.info(
                f"LLMEnforcer initialized: provider={self.client.provider}, "
                f"model={self.client.config.model}"
            )
        else:
            logger.info("LLMEnforcer: No provider available - enforcement disabled")
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "provider": self.client.provider if self.client else "none",
            "model": self.client.config.model if self.client and self.client.config else "unknown",
